...
Markdown under `content/posts` is the single source of truth for all public pages, categories, tags, static post parameters, and public `GET /api/posts` responses. Published content is scanned from the repository at build time. Files with `draft: true` are excluded from public lists and generated post parameters.
...
Administration features still store posts and sessions in `data/blog.sqlite` through `sql.js`, but database posts are no longer used by public pages or public GET APIs. Changes made through the legacy admin editor therefore do not publish website content in this Markdown-only stage. The backend remains temporarily so it can be removed separately after the Markdown workflow is confirmed in production.

The legacy database is written to the local filesystem. Until that backend is removed, ephemeral or read-only serverless filesystems may lose admin/session changes or reject writes. This does not affect public Markdown content, which is bundled from Git during deployment.
...
6. Run `npm run build`.
7. Confirm persistent storage if the transitional administration system will write to SQLite.
8. Connect the GitHub repository to Vercel for automatic push deployments.
9. Connect the domain and verify HTTPS.
...
- The legacy administration editor writes SQLite, not the public Markdown source, during the staged migration.
